document.addEventListener('DOMContentLoaded',()=>{
  ...
  form.addEventListener('submit',async e=>{
    ...
    payload.set('phone',iti?iti.getNumber():phone.value.trim());
    payload.set('subject','Turkish Bath Reservation');
    ...
    payload.set('experience',experience.value);
    payload.set('travelDate',date.value);
    payload.set('preferredTime',time.value);
    payload.set('femaleGuests',String(f));
    payload.set('maleGuests',String(m));
    payload.set('fullName',name.value.trim());
    payload.set('requestDetails',details.value.trim());
    payload.set('referrer','turkish-bath-form');
    const submit=form.querySelector('button[type="submit"]');
    if(submit){submit.disabled=true;submit.dataset.originalText=submit.textContent;submit.textContent='Sending Request...';}
    try{
      const response=await fetch(ENDPOINT,{method:'POST',body:payload});
      const text=(await response.text()).trim();
      if(text==='success'){
        show('Your Turkish bath reservation request has been received.',true);
        form.reset();
        if(iti){iti.setCountry('us');phone.value='';}
        document.getElementById('bathCharCount').textContent='0 / 2000';
        experience.dispatchEvent(new Event('change',{bubbles:true}));
      }else{
        show('Something went wrong. Please try again.');
      }
    }catch(err){
      console.error('Turkish Bath Apps Script error:',err);
      show('Connection error. Please try again later.');
    }finally{
      if(submit){submit.disabled=false;submit.textContent=submit.dataset.originalText||'Submit Reservation Request';}
    }
  },true);
});